...
async function runBrowserQA() {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (!text.includes('[next-auth]') && !text.includes('Download the React DevTools')) {
  ...
  try {
    ...
    await delay(1500); // Allow hydration to complete before typing
    ...
    await page.waitForSelector('[class*="blur-"]', { timeout: 3000 }); // Waits for guest pricing locks to show
    ...
    try {
      ...
      console.log('📸 Saved error state screenshot: error_state.png');
    } catch (e) {

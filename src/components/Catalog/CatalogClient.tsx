...
export default function CatalogClient({ initialProducts, loggedInUser }: CatalogClientProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 w-full font-sans text-left relative min-h-screen animate-fade-in">
      ...
      {/* Title */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6 md:mb-10 border-b border-white/5 pb-4 md:pb-8">
        <div>
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button
            ...
            className="flex items-center justify-center gap-2 py-2.5 px-4 sm:py-2 sm:px-3 md:py-2.5 md:px-4 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 font-semibold hover:border-gold/20 hover:text-white text-xs transition-all w-full md:w-auto"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
        {filteredProducts.map((p) => {
          ...
          return (
            <div key={p.sku_id} className="glass-panel rounded-2xl overflow-hidden glass-card-hover flex flex-col group relative border border-white/5 p-3 md:p-4">
              
              {/* Product image container */}
              <div
                className="relative h-[240px] sm:h-[280px] md:h-[320px] bg-slate-900 cursor-pointer overflow-hidden mb-2"
              ...
              <div className="p-6 flex flex-col gap-4 flex-grow justify-between text-left">
                <div>
                  <h3
                    onClick={() => setSelectedProductDetail(p)}
                    className="font-outfit font-bold text-lg md:text-xl text-white hover:text-gold transition-colors cursor-pointer line-clamp-1"
                  >
                    {p.design_name}
                  </h3>
                ...
                <div className="pt-4 border-t border-slate-900 flex items-center justify-between space-y-2 md:space-y-0">
                  {!isLoggedIn ? (
                    ...
                    <div className="w-full flex flex-col gap-3.5">
                      <div className="flex justify-between items-end">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl md:text-2xl font-extrabold text-gold tracking-tight">₹{p.standard_price} <span className="text-[10px] text-slate-500 font-normal">/ pc</span></span>
                        </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex justify-end">
          <div className="w-full max-w-md bg-slate-900 border-l border-white/5 h-full flex flex-col justify-between shadow-2xl relative animate-scale-in font-sans">
            ...
            <div className="p-5 border-t border-slate-800 bg-slate-900/60 backdrop-blur-md flex flex-col gap-4">
              ...
              {cart.length > 0 && (
                <label className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-950 border border-slate-850 cursor-pointer text-slate-400 select-none text-left">
                  <input
                    type="checkbox"
                    checked={termsAccepted}
                    onChange={(e) => setTermsAccepted(e.target.checked)}
                    className="rounded border-slate-800 text-gold focus:ring-gold accent-gold w-4 h-4 shrink-0 mt-0.5"
                  />
                  <span className="text-[10px] leading-relaxed font-semibold">
                    I accept the terms of sale and parcel opening video requirement.
                  </span>
                </label>
              )}
              ...
              {cart.length > 0 ? (
                <a
                  ...
                  onClick={(e) => {
                    if (!termsAccepted) {
                      e.preventDefault();
                      alert("Please accept the terms of sale and parcel opening video requirement to proceed.");
                      return;
                    }
                  ...
                  className={`w-full py-3 px-4 rounded-xl text-white font-bold text-center flex items-center justify-center gap-2 transition-all shadow-lg text-xs ${
                    termsAccepted 
